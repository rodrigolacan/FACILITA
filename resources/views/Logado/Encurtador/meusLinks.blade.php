...
<div class="container pt-20 mx-auto flex flex-wrap flex-col md:flex-row items-center">
    ...
    <div class="flex flex-col w-full md:w-2/5 justify-center items-start text-center md:text-left">
      ...
      <h1 class="my-4 text-5xl font-bold leading-tight">
        Por partes da URL encurtada, ou original
      </h1>
    </div>
  </div>
...
<div class="w-full h-full p-6 bg-white">
    <form method="GET" action="{{route('buscarMeusLinks')}}" class="max-w-md w-1/2 mx-auto mt-8">   
        <label for="default-search" class="mb-2 text-sm font-medium text-white sr-only dark:text-white">Search</label>
        <div class="relative">
            <input type="search" name="busca-meus-links" id="busca-meus-links" value="{{ request('busca-meus-links') }}" class="block w-full p-4 ps-10 text-sm text-white border border-gray-300 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white" placeholder="Procure por partes da url encurtada ou original"/>
            ...
            <tbody>
               @foreach ($links as $link)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        <td class="px-6 py-4 font-medium text-blue-500 whitespace-nowrap  hover:underline">
    ...
    {{$links->links('vendor.pagination.simple-tailwind')}}
</div>
@endsection